...
const Productmanagement = () => {
  ...
  const { name, price, category, stock, photo } = product;
  ...
  const submitHandler = (e: FormEvent<HTMLFormElement>): void => {
    ...
    if (stockUpdate != 0) formData.set("stock", stockUpdate.toString());
    if (photoFile) formData.set("photo", photoFile);
    if (categoryUpdate) formData.set("category", categoryUpdate);

    updateProduct({ formData, userId: user?._id!, productId: params.id! })
      .unwrap()
      .then((res) => {
  ...
  const deleteHandler = () => {
    deleteProduct({ userId: user?._id!, productId: params.id! })
      .unwrap()
      .then((res) => {
        toast.success(res.message);
        navigate("/admin/product");
      ...
      {isLoading ? (
        ...
        <main className="product-management">
          <section>
            ...
            <img src={`${serverUrl}/${photo}`} alt="Product" />
            <p>{name}</p>
            {stock > 0 ? (
